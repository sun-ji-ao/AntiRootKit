...
/** @brief 确保进程内 Winsock 已初始化（不 Cleanup，避免干扰其他模块）。 */
...
/** @brief 判定当前系统是否为 Windows 10 及以上。 */
...
/**
 * @brief 将 after 快照中未出现在 front 的条目并入 front（R3 并集，降时序误报）。
 */
...
/**
 * @brief 尝试 bind 探测本地地址:端口是否已被占用。
 * @param localAddress 点分 IPv4。
 * @param port 主机序端口。
 * @return 被占用返回 true。
 */
...
/**
 * @brief 通过 IPHLPAPI 未导出函数拉取 Bound TCP/TCP6 端点表。
 * @param boundNetInfo 输出列表。
 * @return ERROR_SUCCESS 或 Win32 错误；函数缺失时返回成功且列表为空。
 */
...
/**
 * @brief 执行跨视图隐藏端口检测（实现）。
 *
 * 流程：R3前 → 内核 AFD/NSI → R3后并集 → Hidden=NSI−R3 → FilterHiddenPorts。
 */
